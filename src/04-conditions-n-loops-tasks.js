...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const condition1 = rect1Right <= rect2.left;
  const condition2 = rect1Bottom <= rect2.top;
  const condition3 = rect1.left >= rect2Right;
  const condition4 = rect1.top >= rect2Bottom;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (bracketPairs[str[i]]) {
      stack.push(str[i]);
    } else if (Object.values(bracketPairs).includes(str[i])) {
      if (stack.length === 0 || bracketPairs[stack.pop()] !== str[i]) {
        return false;
      }
    }
  }
